...
// Login
router.get("/signin", noAuth, (req, res) => {
  res.render("signin", { session: req.session, response: res });
});
...
//Forget password
router.get("/forgetPassword", noAuth, (req, res) => {
...
//filteration
router.get("/3DayTour", (req, res) => {
  user.Display(
    req,
    res,
    "user/Alltours",
    "Select * from tour where tourDuration=3",
    "Tour",
    "3DayTour"
  );
});
router.get("/5DayTour", (req, res) => {
  user.Display(
    req,
    res,
    "user/Alltours",
    "Select * from tour where tourDuration=5",
    "Tour",
    "5DayTour"
  );
});
router.get("/7DayTour", (req, res) => {
  user.Display(
    req,
    res,
    "user/Alltours",
    "Select * from tour where tourDuration=7",
    "Tour",
    "7DayTour"
  );
});
...
// contact us
router.get("/contactUs", [Auth, userAuth], (req, res) => {
...
router.get("/admin/customOrders", [Auth, adminAuth], (req, res) => {
  user.Display(
    req,
    res,
    "admin/displayCustOrders",
    "Select * from customorder",
    "CustomOrder"
  );
});
router.get("/admin/Orders", [Auth, adminAuth], (req, res) => {
  user.Display(
    req,
    res,
    "admin/displayOrders",
...
router.get("/admin/Feedback", [Auth, adminAuth], (req, res) => {
  user.Display(
    req,
    res,
...
router.get("/admin/insert", [Auth, adminAuth], (req, res) => {
  res.render("admin/inputTour");
});
...
router.post(
  "/admin/insert",
  upload1.single("CardImage"),
  upload2.single("BannerImage"),
  admin.insertTour
);
